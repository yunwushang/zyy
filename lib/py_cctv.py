...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def homeVideoContent(self):
		videos=[]
		videos=self.categoryContent(tid='节目大全',pg='1',filter=False,extend={})
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		if tid=='动画片':
			...
			if 'datapd-channel' in extend.keys():
				tem=extend['datapd-channel'].split('#')
				tem[1]=urllib.parse.quote(tem[1])
				temporary=[tem[1]+',','%E9%AB%98%E6%B8%85,',tem[1]+'%E9%AB%98%E6%B8%85']
				for v in temporary:
					channel=channel+tem[0]+v
			if 'datafl-sc' in extend.keys():
				datafl=urllib.parse.quote(extend['datafl-sc'])
	...
	def searchContent(self,key,quick):
		...
		data="type=video&url=&httpflag=https%3A%2F%2F&channel=&qtext_str={0}&qtext={0}".format(key)
		htmlTxt=self.webReadFile_POST(urlStr=Url,data=data,header=headers)
		# form={
		# 	"type":"video",
		# 	"url":"httpflag=https%3A%2F%2F",
		# 	"channel":"",
		# 	"qtext_str":key,
		# 	"qtext",key
		# }
		# rsp = requests.post(url,headers=headers,data=form)
		# htmlTxt=rsp.text
		videos=[]
	...
	config = {
		...
		"纪录片":[
		{"key":"datapd-channel","name":"频道","value":[{"n":"全部","v":""},{"n":"CCTV-1综合","v":"CCTV-1#综合"},{"n":"CCTV-2 财经","v":"CCTV-2#财经"},{"n":"CCTV-3 综艺","v":"CCTV-3#综艺"},{"n":"CCTV-4中文国际","v":"CCTV-4#中文国际"},{"n":"CCTV-5 体育","v":"CCTV-5#体育"},{"n":"CCTV-6 电影","v":"CCTV-6#电影"},{"n":"CCTV-7国防军事","v":"CCTV-7#国防军事"},{"n":"CCTV-8 电视剧","v":"CCTV-8#电视剧"},{"n":"CCTV-9 纪录","v":"CCTV-9#纪录"},{"n":"CCTV-10科教","v":"CCTV-10#科教"},{"n":"CCTV-11 戏曲","v":"CCTV-11#戏曲"},{"n":"CCTV-12 社会与法","v":"CCTV-12#社会与法"},{"n":"CCTV-13新闻","v":"CCTV-13#新闻"},{"n":"CCTV-14 少儿","v":"CCTV-14#少儿"},{"n":"CCTV-15 音乐","v":"CCTV-15#音乐"},{"n":"CCTV-17农业农村","v":"CCTV-17#农业农村"}]},
	...
	#-----------------------------------------------自定义函数-----------------------------------------------
	#访问网页
	def webReadFile(self,urlStr,header,timeout=200):
		html=''
	...
	#访问网页
	def webReadFile_POST(self,urlStr,data,header,timeout=200):
		html=''
		req = request.Request(url=urlStr, data=bytes(data, encoding='utf8'),headers=header, method='POST')
		with  urllib.request.urlopen(req)  as response:
			html=response.read().decode('utf-8','ignore')#ignore忽悠非法字符
		return html
	#判断网络地址是否存在
	...
	def get_m3u8(self,urlTxt):
		...
		html=self.webReadFile(urlStr=url,header=self.header)
		jo =json.loads(html)
		hls_url = jo['hls_url'].strip()
		html = self.webReadFile(urlStr=hls_url,header=self.header)
		content = html.strip()
		arr = content.split('\n')
		urlPrefix = self.get_RegexGetText(Text=hls_url,RegexText='(http[s]?://[a-zA-z0-9.]+)/',Index=1)
		subUrl = arr[-1].split('/')
		url = urlPrefix + arr[-1]
		resolutionRatio=['2000','1200','850',self.get_RegexGetText(Text=arr[-1],RegexText=r'/([0-9]+)\.m3u8',Index=1)]
		for v in resolutionRatio:
			if v=='':
				continue
			try:
				subUrl[3] =v
				subUrl[-1] = v+'.m3u8'
				tem=urlPrefix + '/'.join(subUrl)
				hdRsp = self.TestWebPage(urlStr=tem,header=self.header)
				if hdRsp == 200:
					return tem
			except:
				pass
	...
	def get_list_search(self,html,key):
		...
		try:
			...
			for vod in nodes:
				...
				title=self.get_RegexGetText(Text=temporary.xpath("string(.)"),RegexText=r'(.+?)(\r|\n)',Index=1)
				# print(title)
		...
		try:
			...
			root = self.html(html)
			nodes = root.xpath('//li/a')
			if len(nodes)>0:
				for vod in nodes:
					title=vod.xpath("./@title")[0]
					url=vod.xpath("./@href")[0]
					img=vod.xpath("./img/@src")
					if len(img)<1:
						continue
					img=img[0]
					if title=='' or url=='':
